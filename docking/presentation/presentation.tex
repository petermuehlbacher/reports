...
\end{figure}
}\only<3-3>{set of bond lenghts, bond angles and dihedral angles; usually only some dihedral angles are allowed to vary though\\~\\useful to alter a molecule in a chemically meaningful way}\only<4-4>{set of cartesian coordinates of every single atom in a molecule\\~\\useful to calculate potentials}
\end{columns}
